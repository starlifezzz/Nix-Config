...
#   覆盖 home/dms-shell/settings.json，然后 rebuild 入库 —— 复制即一致。
#
# 用法: ./scripts/sync-dms-settings.sh
set -euo pipefail

DMS_DIR="/etc/nixos/home/dms-shell"
DST="$DMS_DIR/settings.json"
...
# 同步 DMS 用户文件（设置中心管理——git 仓库做文件同步）
# binds.kdl（快捷键）+ windowrules.kdl（窗口规则）
for f in binds.kdl windowrules.kdl; do
  SRC="$HOME/.config/niri/dms/$f"
  DST_FILE="$DMS_DIR/$f"
  if [ -f "$SRC" ]; then
    cp "$SRC" "$DST_FILE"
    echo "✅ 已同步 $f → $DST_FILE"
  else
    echo "⚠️ 未找到 $SRC（DMS 尚未生成）——跳过 $f"
  fi
done
